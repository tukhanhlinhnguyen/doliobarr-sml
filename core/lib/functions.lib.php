...
function getCommonSubstitutionArray($outputlangs, $onlykey = 0, $exclude = null, $object = null)
{
	...
	if (($onlykey || is_object($object)) && (empty($exclude) || !in_array('object', $exclude))) {
		if ($onlykey) {
			...
			//-------------------------------------------------------------------------------------------------------
		    if (is_object($object)) {
                $substitutionarray['__DEPOSIT_PERCENT__'] = $object->deposit_percent;
                $substitutionarray['__DEPOSIT_PERCENT_LEFT__'] = 100 - $object->deposit_percent;
                $substitutionarray['__DEPOSIT_SUM__'] = round(($object->deposit_percent / 100) * $object->total_ttc, 2);
                $substitutionarray['__DEPOSIT_SUM_LEFT__'] = number_format((float)($object->total_ttc - ($object->deposit_percent / 100) * $object->total_ttc), 2, '.', '');
            }
            //----------------------------------------------------------------------------------------------------------
	...
	// Add your custom substitution variables here
    /*if (is_object($object)) {
        $substitutionarray['__DEPOSIT_PERCENT__'] = $object->deposit_percent;
        $substitutionarray['__DEPOSIT_PERCENT_LEFT__'] = 100 - $object->deposit_percent;
        $substitutionarray['__DEPOSIT_SUM__'] = number_format((float)$object->deposit_amount, 2, '.', '');
        $substitutionarray['__DEPOSIT_SUM_LEFT__'] = number_format((float)($object->total_ttc - $object->deposit_amount), 2, '.', '');
    } else {
        $substitutionarray['__DEPOSIT_PERCENT__'] = '__DEPOSIT_PERCENT__';
        $substitutionarray['__DEPOSIT_PERCENT_LEFT__'] = '__DEPOSIT_PERCENT_LEFT__';
        $substitutionarray['__DEPOSIT_SUM__'] = '__DEPOSIT_SUM__';
        $substitutionarray['__DEPOSIT_SUM_LEFT__'] = '__DEPOSIT_SUM_LEFT__';
    }*/
