 ...
 */
class pdf_customstorm extends ModelePdfExpedition
{
	/**
	...
	{
		// phpcs:enable
		global $user, $conf, $langs, $hookmanager, $db;
		...
		if ($conf->expedition->dir_output) {
			...
			if ($object->specimen) {
				...
				$file = $dir."/BL-".$expref.$nom_societe.".pdf";
			}
				...
				for ($i = 0; $i < $nblines; $i++) {
					...
					if ($this->getColumnStatus('photo')) {
						...
						if (!empty($this->cols['photo']) && isset($imglinesize['width']) && isset($imglinesize['height'])) {
							...
							$posYAfterImage = $curY + $imglinesize['height'] +5;
						}
					}
					...
					if ($this->getColumnStatus('qty_asked')) {
						$this->printStdColumnContent($pdf, $curY, 'qty_asked', $object->lines[$i]->qty_shipped);
						$nexY = max($pdf->GetY(), $nexY);
					}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->MultiCell($w, 4, $title." ".$outputlangs->convToOutputCharset($object->ref), '', 'R');
		...
		if (!empty($object->date_delivery)) {
				...
				$pdf->SetXY($posx, $posy);
				$pdf->SetTextColor(0, 0, 60);
				$pdf->MultiCell($w, 4, $outputlangs->transnoentities("Date")." : ".dol_print_date($object->date_delivery, "%d %b %Y", false, $outputlangs, true), '', 'R');
			}
		...
		if (!empty($conf->$origin->enabled)) {     // commonly $origin='commande'
			...
			if ($result >= 0) {
				...
				$sql = "SELECT * FROM " . $this->db->prefix() . "element_element WHERE fk_target = ".$origin_id." AND targettype = 'commande' AND sourcetype = 'propal'";
				$result = $this->db->query($sql);
				$propal = $this->db->fetch_object($result);
				$p = new Propal($this->db);
				$resultp = $p->fetch($propal->fk_source);
				$pdf->SetFont('', '', $default_font_size - 2);
				...
				if($resultp){
					$pdf->MultiCell($w, 2, $outputlangs->transnoentities("RefProposal")." : ".$outputlangs->transnoentities($p->ref), 0, 'R');
					$Yoff = $Yoff + 3;
					$pdf->SetXY($this->page_largeur - $this->marge_droite - $w, $Yoff);
