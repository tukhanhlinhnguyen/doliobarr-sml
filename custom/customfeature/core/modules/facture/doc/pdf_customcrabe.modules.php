 ...
 */
class pdf_customcrabe extends ModelePDFFactures
{
	 /**
		...
		if ($conf->facture->dir_output) {
			...
			if (file_exists($dir)) {
				...
				if ($notetoshow) {
					...
					$this->complete_substitutions_array($substitutionarray, $outputlangs, $object);
		...
		if ($object->type != 2) {
			...
			if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR') {
				if ($object->fk_account > 0 || $object->fk_bank > 0 || !empty($conf->global->FACTURE_RIB_NUMBER)) {
					...
					$posy = $this->pdf_bank($pdf, $outputlangs, $curx, $cury, $account, 0, $default_font_size);

					$posy += 2;
				}
			}

			// If payment mode not forced or forced to FACTOR
			if ($object->mode_reglement_code == 'FACTOR') {

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', 'Pour être libératoire, le règlement de cette facture doit être effectué et adressé à : BPCE FACTOR Tél : 01 58 32 80 00', 0, 1);
				$posy = $pdf->GetY() + 0.1;

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', '5, avenue de la Liberté - 94676 Charenton-le-Pont Cedex ', 0, 1);
				$posy = $pdf->GetY() + 0.1;

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', 'RIB : 30007 00011 00010500791 36 - Domiciliation : NATIXIS Paris ', 0, 1);
				$posy = $pdf->GetY() + 0.1;

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', 'IBAN : FR76 3000 7000 1100 0105 0079 136 ', 0, 1);
				$posy = $pdf->GetY() + 0.1;

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', 'SWIFT CODE (BIC) : NATX FR PP XXX ', 0, 1);
				$posy = $pdf->GetY() + 0.1;

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', 'N° ICS : FR57ZZZ483693 ', 0, 1);
				$posy = $pdf->GetY() + 0.1;

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', 'BPCE FACTOR a acquis notre créance par voie de subrogation dans le cadre d’un contrat d’affacturage.', 0, 1);
				$posy = $pdf->GetY() + 0.1;

				$pdf->SetXY($this->marge_gauche, $posy);
				$pdf->writeHTMLCell(90, 3, '', '', 'BPCE FACTOR devra être avisée de toute réclamation', 0, 1);
				$posy = $pdf->GetY() + 0.1;
			}
		}
	...
	protected function _pagefoot(&$pdf, $object, $outputlangs, $hidefreetext = 0)
	{
		global $conf;
		if($object->mode_reglement_code == 'FACTOR'){
			return pdf_pagefoot($pdf, $outputlangs, '', $this->emetteur, $this->marge_basse, $this->marge_gauche, $this->page_hauteur, $object, $showdetails, $hidefreetext, $this->page_largeur, $this->watermark);
		}
		else {
...
function complete_substitutions_array(&$substitutionarray, $outputlangs, $object = null, $parameters = null, $callfunc = "completesubstitutionarray")
{
    ...
    if (is_object($object)) {
        $substitutionarray['__DEPO_PERCENT__'] = $object->deposit_percent;
    } else {
        $substitutionarray['__DEPO_PERCENT__'] = '__DEPO_PERCENT__';
    }
